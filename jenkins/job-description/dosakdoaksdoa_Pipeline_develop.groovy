...
def job_config = [
        job: [
                name: "dosakdoaksdoa_Pipeline_develop",
                agent: "maven"
        ],
        git: [
                branch: "develop"
...
def customLib = library identifier: 'dosakdoaksdoa_JSL@develop', retriever: modernSCM(
  [\$class: 'GitSCMSource',
   remote: 'https://github.developer.allianz.io/kkanto/dosakdoaksdoa_lib.git',
   credentialsId: 'git-token-credentials']) 
...
def jslCustom     = customLib.de.allianz.dosakdoaksdoa.new()
    ...
    stages {
        ...
        stage('Checkout') {
            steps {
                echo "checkout"
                script {
                   jslGit.checkout( config, "kkanto", "dosakdoaksdoa", job_config.git.branch)
               }
            }    
        }
        stage('Build') {
...
def job = pipelineJob("dosakdoaksdoa_Pipeline_develop");

job.with {

    authenticationToken('6FXVmSkmWNTvQqm9KsQPeqScC4mTbffM')
